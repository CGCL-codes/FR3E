...
from relation_model.fr3e import FR3E
from relation_model.callback import MyCallBack
from relation_model.data import load_data, get_data_iterator
from relation_model.config import Config
from relation_model.evaluate import metric
...
parser.add_argument('--dataset', default='finance_add_predictNer', type=str, help='define your own dataset names')
...
parser.add_argument('--h_bar', default=0.5, type=float)
...
if __name__ == '__main__':
    ...
    precision, recall, f1_score = metric(test_data, rel_vocab, con, model, h_bar=con.h_bar, t_bar=con.t_bar)
    with open(os.path.join(con.save_logs_dir, con.log_save_name), 'a+') as f_log:
        f_log.write('batch size: {}, h_bar:{}, t_bar:{}, f1: {:4.4f}, precision: {:4.4f}, recall: {:4.4f}\n\n'
                .format(con.batch_size, con.h_bar, con.t_bar, f1_score, precision, recall))
